...
import { Meeting, deleteMeeting, listMeetings, uploadMeeting } from "@/lib/api";
        ...
        <div className="flex-1 overflow-y-auto p-2">
          ...
          {!loadError && meetings.length === 0 && (
            <p className="px-2 py-4 text-center text-xs text-zinc-600">
              No meetings yet.<br />Upload an audio or video recording to get started.
            </p>
          )}
          <div className="flex flex-col gap-2">
            {meetings.map((m) => (
              <MeetingCard key={m.id} meeting={m} selected={selected?.id === m.id} onSelect={setSelected} onDelete={handleDelete} />
            ))}
          </div>
        </div>
      </aside>
